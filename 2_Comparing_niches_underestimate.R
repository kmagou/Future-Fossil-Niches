...
##### Load data 
#Basin overlap data
basinData<-read.csv("/Users/Marshall_Lab/Desktop/Chapter_3/Kat_data/modern_climate_contiguous_US_BASIN_OVERLAP_underestimate.csv")
...
#write_csv(subSpecies, "species_counts_underestimate.csv")

test<-subSpecies[subSpecies$basins>=20,]
test<-na.omit(test)
test
nrow(test)
...
setwd("/Users/Marshall_Lab/Desktop/Chapter_3/Kat_data/Overlap_results_D_underestimate")
...
(densityBasins_plot <- densityBasins %>% 
    ggplot(aes(x = x, y = y, color = density)) +
    geom_point(alpha = 0.2) +
    scale_color_gradient(low = "skyblue3", high = "black") +
...
counts = data.frame(test)
resultsAnaly <- rename(resultsAnaly,c("species"="speciesAnal"))
final_data <- merge(resultsAnaly,counts,by="species")
...
# Save results
write.csv(final_data,"Similarity_Basins-NoBasins_underestimate.csv")
