...
modernData<-read.csv("C:/Users/au807879/OneDrive - Aarhus universitet/Desktop/More_Ch3/modern_climate_contiguous_US.csv")
...
basinData1<-read.csv("C:/Users/au807879/OneDrive - Aarhus universitet/Desktop/More_Ch3/modern_climate_contiguous_US_BASIN_OVERLAP.csv")
...
basinData2<-read.csv("C:/Users/au807879/OneDrive - Aarhus universitet/Desktop/More_Ch3/modern_climate_contiguous_US_BASIN_OVERLAP_underestimate.csv")
...
write.csv(final_data_over,"final_data_over_hulls.csv")

write.csv(final_data_under,"final_data_under_hulls.csv")
...
(lm_plot_over <- ggplot(final_data_over, aes(x = area_km2, y = Overlap)) +
    ...
    labs(x = bquote(Size~of~present~convex~hull~(km^2)), y = "Overlap (Schoener's *D*)") +
    theme_light() +
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 16), axis.title.y = ggtext::element_markdown()) +
    lims(x = c(0, 10150000), y = c(0, 1)))
